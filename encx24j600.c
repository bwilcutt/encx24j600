 ...
 *
 * Bryan Wilcutt <bwilcutt@yahoo.com>
 *
 * Modifications to work with Device Tree.
 *
 * Sample device tree entries:
...
static void encx24j600_hw_init_tx(struct encx24j600_priv *priv)
{
	...
	encx24j600_clr_bits(priv, EIR, TXIF | TXABTIF | LINKIF);	
...
static int encx24j600_spi_probe(struct spi_device *spi)
{
	...
	if (IS_ERR(priv->kworker_task)) {
		...
		netif_err(priv, probe, ndev, "encx24j600: Could not run thread (%d)\n", ret);
		goto out_free;
	}
	...
	// Link predefined GPIO lin (P8_26) to our IRQ.

	valid = gpio_is_valid(GPIO1_29);

	if (valid)
	{
		status = gpio_request(GPIO1_29, "GPIO1_29");
	
		if (status >= 0)
		{
			priv->ctx.spi->irq = gpio_to_irq(GPIO1_29);
			ret = request_threaded_irq(priv->ctx.spi->irq, NULL, encx24j600_isr,
				       IRQF_TRIGGER_FALLING | IRQF_ONESHOT, DRV_NAME, priv);
		}
	}
